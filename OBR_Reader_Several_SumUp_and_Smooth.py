...
from ReaderFunctions.ReaderFunctions import smooth as smt
import os
...
for i in range(0,len(highest_numbers)):
    ...
    filename="Output_Summed_Smooth_"+str(prefix)+str(start)+"_"+str(stop)+".txt"
    ...
    for i in range(start,stop+1,1):
        ...
        StrainSummedSmooth=smt.SmootherAvg(Length,StrainSummed)
        AllStrain.append(StrainSummedSmooth)
