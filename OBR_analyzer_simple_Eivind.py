...
while not correct_input:
	try:
		message_1 = 'Number of first file to be analyzed: '
...
while not correct_input:
	try:
		message_1 = 'Running reference? answer "yes" or "no": '
		RunningReference = str(raw_input(message_1))
		if RunningReference == "yes":
			RRef=True
			correct_input = True
		elif RunningReference == "no" :
			RRef=False
			correct_input = True
	except ValueError:
		pass
...
if not RRef:
	while not correct_input:
		try:
			message_1 = 'Reference file number: '
			Refnum = int(raw_input(message_1))
			if Refnum >= 0:
				correct_input = True
		except ValueError:
			pass
